...
//            cout <<"number of actor " <<actorNumofMovieInsert[j-1]<<endl;
    ...
    float my_weight;
    for( int k = 0; k<3; k++){
    map<float, int> neighbor;
        for(map<int,vector<weight> >::iterator it=weightMap.begin(); it!=weightMap.end(); it++){
            if(it->first == nodeID[k]){
                for(int i=0; i<(it->second).size();i++){
                    my_weight = (it->second)[i].w / (actorNumofMovie[it->first]+actorNumofMovie[(it->second)[i].id2]-(it->second[i].w));
                    neighbor[my_weight] = (it->second)[i].id2;
                }
            }
            else{
                for(int i=0; i<(it->second).size();i++){
                    if((it->second)[i].id2 == nodeID[k]){
                        my_weight = (it->second)[i].w / (actorNumofMovie[it->first]+actorNumofMovie[(it->second)[i].id2]-(it->second[i].w));
                        neighbor[my_weight] = (it->first);
                    }
                }
            }
        }
        
        ofstream outfile("/Users/luanzhang/Dropbox/ucla/EE232/project_2/result/6/5_neighbors_15_1.txt", ios::out | ios::app);
        if (outfile.fail())
        {
        ...
        outfile << nodeID[k] <<"'s five nearest neighbors: " <<endl;
        
        map<float, int >:: iterator it=neighbor.begin();
        for(int i = 0; i<5; i++){
            if(it != neighbor.end()){
...
//    ofstream outfile("/Users/luanzhang/Dropbox/ucla/EE232/project_2/movie_undirected_30_insert.txt", ios::out);
//    if (outfile.fail())
//    {
//        cout << "Error opening the file ";
//        exit(1);
...
//    
//    
//    ofstream movieID("/Users/luanzhang/Dropbox/ucla/EE232/project_2/movieID_30_insert.txt", ios::out);
//    if (movieID.fail())
//    {
//        cout << "Error opening the file ";
//        exit(1);
